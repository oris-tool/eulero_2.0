...
import org.oristool.eulero.graph.And;
import org.oristool.eulero.graph.DFSObserver;
import org.oristool.eulero.graph.Or;
import org.oristool.eulero.graph.Seq;
...
public class PetriBuilder {
    ...
    public static void build(Activity root) {
        ...
        // for replicated dependencies (we add them once)
        ...
        root.dfs(new DFSObserver() {
            ...
            @Override public void onClose(Activity closed) {
                ...
                if (closed instanceof Analytical) {
                    ...
                    Place prev = null;
                    for (int i = 0; i < closed.dependencies().size(); i++) {
                        Transition tdep = tmap.get(closed.dependencies().get(i));
                                
                        if (prev != null)
                            net.addPrecondition(prev, tdep);
                        
                        Place next = (i < closed.dependencies().size() - 1) 
                    ...
                    for (Activity dep : closed.dependencies()) {
