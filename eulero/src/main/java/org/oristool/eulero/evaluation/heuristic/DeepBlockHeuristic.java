...
public class DeepBlockHeuristic extends AnalysisHeuristicStrategy{
    public DeepBlockHeuristic(BigInteger CThreshold, BigInteger SThreshold, Approximator approximator, boolean verbose) {
        super("Deep Block", CThreshold, SThreshold, approximator, verbose);
    }
        ...
        if(model instanceof DAG) {
            ...
            if (!(model.simplifiedC().compareTo(model.C()) == 0) || !(model.simplifiedQ().compareTo(model.Q()) == 0)) {
                if(model.C().compareTo(this.CThreshold()) > 0 || model.Q().compareTo(this.QThreshold()) > 0){
                    ...
                    return DAGInnerBlockAnalysis2(model, timeLimit, step, forwardReductionFactor, error, tabSpaceChars);
                }
